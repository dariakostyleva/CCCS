...
void draw_transm_x2(){
    ...
    std::ifstream file_in("s533_51_gC_12mm_0090_transm_x2.txt");
    std::ifstream file_out("s533_51_empty_0086_transm_x2.txt");
    Int_t fitlim1 = -9;
    Int_t fitlim2 = 9;
    ...
    // Draw the graph_in
    TCanvas* c = new TCanvas("c", "graph_in with Errors", 800, 600);
    c->Divide(2,1);
    c->cd(1);
    graph_in->Draw("AP");  
    graph_out->Draw("P same");// "A" for axis, "P" for points with errors
