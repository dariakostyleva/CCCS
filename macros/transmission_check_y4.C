...
void transmission_check_y4(){
	...
	tree->SetBranchAddress("tpc_y_41",&tpc_y_41);
  	...
  	Float_t id_z1_c1;
  	Float_t id_z1_c2;
  	fileTransmOut.Form("%s_transm_y4.txt",name1);
	...
	TH2F *h_id_cut_tpc = new TH2F("h_id_cut_tpc","h_id_cut_tpc",500,1.5,2.5,500,4,10);
  	...
  	TH1F *h_id_y4 = new TH1F("h_id_y4","h_id_y4",100,-50,50);
  	TH1F *h_id_y4_slice = new TH1F("h_id_y4_slice","h_id_y4_slice",100,-50,50);


  	printf("Total number of events is %lld \n",nentries);

  	//loop over TPC cut condition
  	for (int k = -25; k <= 30; k+=5) {
  		tpc_cut_c1 = k;
  		tpc_cut_c2 = k+5;
		...
		for (Int_t i=0;i<nentries;i++) {
			...
			if(//id_x2 > -9 && id_x2 < 9 &&
				...
				tpc_y_41 > tpc_cut_c1 && tpc_y_41 < tpc_cut_c2){
